...
function SensorOptionsPanel({ sensor, options, searchQuery, isExpanded, onToggle, onSetOption }: SensorOptionsPanelProps) {
  ...
  // Same order the C++ viewer draws these sections in (device-model.cpp): the sensor's
  // own controls, then advanced mode, then the colorizer, then the filter chain.
  const categoryOrder = ['Basic Controls', 'Advanced Controls', 'Depth Visualization', 'Post-Processing']
